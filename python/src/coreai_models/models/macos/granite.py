...
class TransformerBlock(nn.Module):
    def __init__(self, config: GraniteConfig, layer_idx: int) -> None:
        ...
        # Granite-specific: residual multiplier applied after each sub-block
    ...
    def forward(
        ...
        h = x + r * self.residual_multiplier  # Granite: scale residual
        r = self.mlp(self.post_attention_layernorm(h))
        return h + r * self.residual_multiplier  # Granite: scale residual
...
class GraniteModel(nn.Module):
    def __init__(self, config: GraniteConfig) -> None:
        super().__init__()
        hidden_size = config.hidden_size
        # Granite-specific: embedding multiplier applied after embedding lookup
    ...
    def forward(
        ...
        h = self.embed_tokens(input_ids) * self.embedding_multiplier  # Granite: scale embeddings
        for layer in self.layers:
            h = layer(h, position_ids, cache)
        return self.norm(h)


class GraniteForCausalLM(BaseForCausalLM):
    ...
    def forward(
        ...
        return logits / self.config.logits_scaling  # type: ignore[union-attr]
    ...
    def _mutate_state_dict(self: Self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        for i in range(max_layer + 1):
            # Fuse q/k/v projections into combined qkv_proj (like Mistral/Qwen2)
            combined_weight = []
            need_to_fuse = True
