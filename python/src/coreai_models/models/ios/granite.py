...
class Attention(nn.Module):
    def __init__(self, config: GraniteConfig, layer_idx: int) -> None:
        ...
        # Granite-specific: attention_multiplier as SDPA scale
...
class GraniteModel(nn.Module):
    def __init__(self, config: GraniteConfig) -> None:
        super().__init__()
        hidden_size = config.hidden_size
        # Granite-specific: embedding multiplier
...
class GraniteExtend(nn.Module):
    def __init__(self, config: GraniteConfig):
        ...
        # Granite-specific: store logits_scaling for application in forward
    ...
    def forward(
        ...
        if self.lm_head is not None:
            ...
            # Reverse LM head with embedding table
...
class GraniteForCausalLMForiOS(BaseForCausalLMForiOS):
    ...
    def _mutate_state_dict(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        # Granite-specific: store logits_scale
        logits_scale_val = 1.0 / self.config.logits_scaling  # type: ignore[union-attr]
