...
"""Tests for macOS Granite dense model parity with HuggingFace."""
...
class TestmacOSGraniteForCausalLM:
    """Test macOS GraniteForCausalLM against HuggingFace reference."""

    def test_forward_parity_single_token(self):
        """Single-token decode: our macOS model matches HF logits."""
        hf_config = _make_granite_config()
        our_config = _make_granite_config()

        torch.manual_seed(42)
        hf_model = HFGraniteForCausalLM(hf_config).to(torch.float32).eval()

        our_model = GraniteForCausalLM(our_config, model_device="cpu")
    ...
    def test_forward_parity_multi_layer(self):
        ...
        hf_config = _make_granite_config(num_hidden_layers=3)
        our_config = _make_granite_config(num_hidden_layers=3)
    ...
    def test_forward_parity_float16(self):
        ...
        torch.manual_seed(42)
        hf_model = HFGraniteForCausalLM(hf_config).to(torch.float16).eval()
    ...
    def test_mutate_state_dict_fuses_qkv(self):
        ...
        our_config = _make_granite_config(num_hidden_layers=1)
        our_model = GraniteForCausalLM(our_config, model_device="cpu")
        ...
        print("State dict mutation: q/k/v fused into qkv_proj - OK")

    def test_forward_parity_real_config(self):
        """Verify with real model config parameters (rope_theta=1e7, rms_norm_eps=1e-5)."""
        hf_config = _make_granite_config(
            rope_theta=1e7,
            rms_norm_eps=1e-5,
            num_hidden_layers=2,
        ...
        max_diff = (our_out - hf_out.logits).abs().max().item()
        mean_diff = (our_out - hf_out.logits).abs().mean().item()
        print(f"Real config (f32, seq=6): max_diff={max_diff:.8f}, mean_diff={mean_diff:.8f}")
        torch.testing.assert_close(our_out, hf_out.logits, atol=1e-5, rtol=1e-5)
    ...
    def test_forward_parity_real_config_f16(self):
        ...
        max_diff = (our_out - hf_out.logits).abs().max().item()
        mean_diff = (our_out - hf_out.logits).abs().mean().item()
        print(f"Real config (f16, seq=6): max_diff={max_diff:.8f}, mean_diff={mean_diff:.8f}")
        torch.testing.assert_close(our_out, hf_out.logits, atol=5e-3, rtol=5e-3)
